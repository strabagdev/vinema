...
describe("local sync repositories", () => {
  ...
  it("enqueues node create, update, archive and restore with previous baseVersion", async () => {
    ...
    await archiveNode(repositories.nodeRepository, node.id, device);
    await restoreNode(repositories.nodeRepository, node.id, device);
  ...
  it("enqueues context and relation writes without exposing mutations to UI", async () => {
    ...
    const updated = await updateContext(repositories.contextRepository, {
      id: context.id,
      name: "Trabajo profundo",
    });
    await archiveContext(repositories.contextRepository, updated.id);
    await restoreContext(repositories.contextRepository, updated.id);
    ...
    await expect(outbox.getById("88888888-8888-4888-8888-888888888888")).resolves.toMatchObject({
      mutation: {
    ...
    await expect(outbox.getById("99999999-9999-4999-8999-999999999999")).resolves.toMatchObject({
      mutation: {
        entityType: "captureConcept",
        entityId: relation.id,
        baseVersion: 1,
